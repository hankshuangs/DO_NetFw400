	...
	public partial class Form1 : Form
	{
		...
		private void ToBak(string parParameter)
		{
			try
			{
				...
				if (lstFiles1.Count != 0 && FtpAutoUpload.Properties.Settings.Default.isbakComplete == (int)Inf.complete)
				{
					FtpAutoUpload.Properties.Settings.Default.isbakComplete = (int)Inf.complete; //備份完成
					WriteLog(baseDir1 + "備份完成");
				}
				else if (lstFiles1.Count == 0 && FtpAutoUpload.Properties.Settings.Default.isbakComplete == (int)Inf.complete)
				{
					WriteLog(baseDir1 + "中無行車記錄檔");
				}

				if (lstFiles1.Count != 0 && FtpAutoUpload.Properties.Settings.Default.isbakComplete == (int)Inf.fail)
				{
					WriteLog(baseDir1 + "本機備份完成，但移至FTP檔可能存在");
				}
				...
				if (lstFiles2.Count != 0 && FtpAutoUpload.Properties.Settings.Default.isbakComplete == (int)Inf.complete)
				{
					FtpAutoUpload.Properties.Settings.Default.isbakComplete = (int)Inf.complete; //備份完成
					WriteLog(baseDir2 + "備份完成");
				}
				else if (lstFiles2.Count == 0 && FtpAutoUpload.Properties.Settings.Default.isbakComplete == (int)Inf.complete)
				{
					WriteLog(baseDir2 + "中無行車記錄檔");
				}
				if (lstFiles2.Count != 0 && FtpAutoUpload.Properties.Settings.Default.isbakComplete == (int)Inf.fail)
				{
					WriteLog(baseDir2 + "本機備份完成，但移至FTP檔可能存在");
				}
				WriteLog("備份結束");
			}
			catch (Exception ex)
		...
		public void Autotrak(ArrayList lstFiles)
		{
			...
			if (lstFiles.Count==0)
			{
				FtpAutoUpload.Properties.Settings.Default.isbakComplete = (int)Inf.complete;
				FtpAutoUpload.Properties.Settings.Default.Save();
			}
		}
			...
			try
			{
				if (Directory.Exists(strPath))
				{
					...
					foreach (var item in files)
					{
						...
						if (Path.GetExtension(item).Equals(".jas") || Path.GetExtension(item).Equals(".SDT") || Path.GetFileName(item).Equals(DateTime.Now.ToString("yyyyMMdd") + ".txt"))
						{
							arrlist.Add(item);
						}
					}
		...
		public void CreateFTP_Forder(string sFullPath, string sUserName, string sPassword)
		{
			...
			{
				string eaa = ex.ToString();
				if (ex.ToString().IndexOf("550") != -1)
				{
					//建立目錄若出現550例外不做處理

				}
				if (ex.ToString().IndexOf("無法連接至遠端伺服器") != -1)
				{
					WriteLog("無法連結至遠方伺服器，請查看FTP是否正常");
					FtpAutoUpload.Properties.Settings.Default.isbakComplete = (int)Inf.fail;
					FtpAutoUpload.Properties.Settings.Default.Save();
					throw;
				}
